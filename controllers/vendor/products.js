...
const createProduct = async(req,res)=>{
    ...
    try{
        ...
        if (!token) {
          ...
          if(!OneVendor){
            return res.status(StatusCodes.NOT_FOUND).json({msg:'Vendor does not exist,cannot proceed'})
          }
          ...
          return res.status(StatusCodes.CREATED).json({msg:'The product was created successfully', newProduct})
