...
const getAllPayments = async(req,res)=>{
    ...
    try{
        ...
        const latestPayment = await PaymentModel.findOne({}).sort({createdAt:-1}).exec()

        if(!latestPayment){
            return res.status(StatusCodes.NOT_FOUND).json({msg:'No payments found'})
        }

        return res.status(StatusCodes.OK).json({msg:'Latest Payment is:, latestPayment'})
